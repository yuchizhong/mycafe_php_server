...
$result = mysql_query("SELECT * FROM dishes, orders, stores, orderDetails WHERE customerID='$customerID' AND orders.storeID='$store' AND payFlag=0 AND orderFlag=0 AND orders.storeID=stores.storeID AND dishes.storeID=orders.storeID AND dishes.dishID=orderDetails.dishID AND orderDetails.orderID=orders.orderID ORDER BY orders.orderID DESC, orderDetails.dishID ASC");
$arrlist = array();
while ($row = mysql_fetch_array($result)) {
